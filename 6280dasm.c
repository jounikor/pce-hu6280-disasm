/*****************************************************************************
    ...
    A commandline disassembler modification (c) Jouni Korhonen based on
    the stuff below. Thanks ;-)
...
#define DASMFLAG_STEP_OVER 0x40000000
#define DASMFLAG_STEP_OUT  0x20000000
 ...
 *****************************************************************************/
static int cpu_disassemble( uint8_t *oprom, char *buffer, int pc  )
{
	uint8_t *opram = oprom;
    uint32_t flags = 0;
	int PC, OP, opc, arg;
...
static int do_disasm(uint8_t *rom, int sze, int sta, int off, int len) {
    char str[32];
    int i,n,m;
    ...
    while (len > 0) {
        n = cpu_disassemble( rom+off, str, sta ); 
        m = (n & 0x00ffffff);
        
        printf("<%02x:%04x>  %-24s ",sta >> 16, sta & 0xffff,str);

        for (i = 0; i < m; i++) {
            printf("%02x ",rom[off+i]);
        }
        for (; i < 7; i++) {
            printf("   ");
        }
        printf(" '");
        for (i = 0; i < m; i++) {
            printf("%c",isprint(rom[off+i]) ? rom[off+i] : '.');
        }
        printf("'\n");
...
static void usage( int a, char **v ) {
    fprintf(stderr,"Hu6280 disassembler v0.1 (c) 2013 Jouni 'Mr.Spiv' Korhonen\n"
        "Based on the MAME's Portable HuC6280 emulator by Bryan McPhail\n\n"
        "Usage: %s [options] file \n\n"
        "  -o offset  Start disassembling from 'offset' of the file (default 0).\n"
        "  -l length  Disassemble 'length' octets (default complete file).\n"
        "  -s start   Base address for the disassembly (default 0xe000)\n\n"
...
int main( int argc, char **argv ) {
    ...
    while ((c = getopt(argc, argv, "s:o:l:")) != -1) {
        switch (c) {
        case 's':
            staAddr = strtol(optarg,NULL,16);
            printf("staAddr: %s = %x\n",optarg,staAddr);
            break;
        case 'o':
    ...
    if ((ifh = fopen(ifname,"r"))) {
        ...
        if (offset < flen) {
            ...
            if ((rom = malloc(flen+7))) {
                if (fread(rom,1,flen,ifh) == flen) {
                    do_disasm(rom,flen,staAddr,offset,length);
                    success = 1;
                } else {
            ...
            fprintf(stderr,"**Error: offset greater than the ROM size.\n");
        }
